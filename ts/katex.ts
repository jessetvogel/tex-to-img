...
export function katexTypeset(tex: string, onError: (error: string) => void): string {
    if (!('katex' in window))
        throw new Error(`Missing object 'katex'`);

    const katex = window.katex as any;
    ...
    try {
        ...
        if (err instanceof katex.ParseError) {
            ...
            const html = (err as any).message.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            onError(html);
            return "";
        } else {
            throw err;
